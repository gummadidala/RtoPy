...
def get_det_config_factory(bucket: str, folder: str):
    ...
    def get_det_config(date_: Union[str, date, datetime]) -> pd.DataFrame:
        ...
        try:
            ...
            # Look for feather files first, then fall back to CSV
            feather_files = [obj for obj in response['Contents'] if obj['Key'].endswith('.feather')]
            csv_files = [obj for obj in response['Contents'] if obj['Key'].endswith('.csv')]
            
            if feather_files:
                # Use feather file
                config_obj = feather_files[0]
                s3_key = config_obj['Key']
                
                logger.info(f"Reading feather file from S3: bucket={bucket}, s3_key={s3_key}")
                
                # Read feather file from S3
                df = s3read_using(
                    pd.read_feather,
                    bucket=bucket,
                    object=s3_key
                ...
                logger.info(f"Successfully read detector config from feather file")
                
            elif csv_files:
                # Fall back to CSV with encoding handling
                config_obj = csv_files[0]
                s3_key = config_obj['Key']
                
                logger.info(f"No feather file found, falling back to CSV: bucket={bucket}, s3_key={s3_key}")
                ...
                raise ValueError(f"No feather or CSV files found for {date_str}")
